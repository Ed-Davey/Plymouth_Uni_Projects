...
#define SIZE_OF_MAZE 21//set the size of the maze as a constant
int playerX, playerY, lives;//declare player variables
int alien1X, alien1Y, alien2X, alien2Y, alien3X, alien3Y, alien4X, alien4Y, alien5X, alien5Y;//declare alien variables
int alien1Dir, alien2Dir, alien3Dir, alien4Dir, alien5Dir = -1;//declare and initialise alien direction
bool Has_Key = false;//declare boolean flag

std::string array [SIZE_OF_MAZE][SIZE_OF_MAZE];//declare the maze
...
int main(){
    ...
    while (!(playerX == 19 && playerY == 19 && Has_Key)){//when player has got to the end square with the key
        //move the player and all the aliens
        ...
        //std::cout << "Player Position: (" << playerX << ", " << playerY << ")" << std::endl;//debug statement
        //check if player at riddle square
        if (playerX == 1 && playerY == 19) {
            riddle(&Has_Key);//launch riddle function
            if (Has_Key == false) {//end game if user returns without the key
                std::cout << "GAME OVER!" << std::endl;
                return 0;
            } else if (Has_Key == true) {//update the icon on the end square
                array [19][19] = "🔐";
                std::cout << "Finish square updated!" << std::endl;
    ...
    //tell the user they have completed the maze
...
void title_screen(int* lives) {
    ...
    //check is the user enters a valid difficulty level and don't let the program carry on until they do
    while (valid_input != true)
        {
        std::cout << "Select difficulty level Easy (E), Medium (M), Hard (H): "; //print difficulty settings
        std::cin >> input; //get user input
        std::cout << std::endl; //new line

        if (input == "E" || input == "e") { //if user selects easy
            *lives = 5; //set lives to 5
            valid_input = true; //allow the while loop to end
        } else if (input == "M" || input == "m") { //if the user selects medium
            *lives = 3; //set lives to 3
            valid_input = true; //allow the while loop to end
        } else if (input == "H" || input == "h") { //if the user selects hard
            *lives = 1; //set lives to 1
            valid_input = true; //allow the while loop to end
        }else { //tell the user what they put in was wrong and what exactly to input
...
int player_movement(int* playerX,int* playerY, int* lives, bool* Has_Key) {
    ...
    switch (input) {
        case 'W': case 'w': {//if the user selects w
            changeX = *playerX-1;//decrease the x value to make the user go up in the maze
            //std::cout << "up\n";//debug if the statement is working
            break;
        }
        case 'S': case 's': {//if the user selects s
            changeX = *playerX+1;//increase the x value to make the user go down in the maze
            //std::cout << "down\n";//debug if the statement is working
            break;
        }
        case 'A': case 'a': {//if the user selects a
            changeY = *playerY-1;//decrease the y value to make the user go left in the maze
            //std::cout << "left\n";//debug if the statement is working
            break;
        }
        case 'D': case 'd': {//if the user selects d
            changeY = *playerY+1;//increase the y value to make the user go right in the maze
            //std::cout << "right\n";//debug if the statement is working
            break;
        }
    ...
    //ensure the tile the player is moving to isn't a wall
    if (changeX > 0 && changeX < (SIZE_OF_MAZE-1) && changeY > 0 && changeY < (SIZE_OF_MAZE-1) && array[changeX][changeY] != "⬛") {
        if (*playerX == 19 && *playerY == 19 && *Has_Key == false) {
            array[*playerX][*playerY] = "🔒";//leave the correct symbol behind if the player goes over the lock square
        } else if (*playerX == 19 && *playerY == 19 && *Has_Key == true) {
            array[*playerX][*playerY] = "🔐";//leave the correct symbol behind if the player goes over the unlocked square
        } else {
            array[*playerX][*playerY] = "⬜️";//leave the path symbol behind
        }
...
bool riddle(bool* Has_Key) {
    ...
    std::string answer, reply;//declare the string variables
    bool ready = false;//declare the boolean flag

    //ensure the user is ready for the riddles
    while (ready == false) {
        std::cout << "Are you ready to face the riddles?" << std::endl;
        std::cin >> reply;
        if (reply == "yes") {
            ready = true;//allow the program to progress from the while loop
        } else {std::cout << "type 'yes' when you are ready";}
    }
    //reset the input buffer from using cin and getline
    std::cin.ignore();

    while (attempts != 0) {//check the user has attempts left
        int random_num = rand() % 16;//random number from 0 to 15
        //int random_num = 0;//debug statement

        //std::cout << random_num << std::endl;//debug statement
        switch (random_num) {
            case 0: {//if the random number is 0
                std::cout << "What has keys but can't open locks?" << std::endl;//show ridddle
                std::getline(std::cin, answer);//gets users answer
                if (answer == "piano" || answer == "a piano") {//check against answers
                    *Has_Key = true;//update boolean flag
                    std::cout << std::endl;//new line
                    break;
                } else {
                    std::cout << "Incorrect answer" << std::endl;//tell user the got it wrong
                    attempts--;//decrease atempts by 1
                    std::cout << "Number of attempts left: " << attempts << std::endl << std::endl;//show attempts left
                    break;
                }
            }
            case 1: {
        ...
        //break the while loop if the user got one right
        if (*Has_Key == true) {
            break;
        }
    }
...
void alien1_spawn(int *alien1X, int *alien1Y, std::string (&array)[SIZE_OF_MAZE][SIZE_OF_MAZE]) {
    bool alien_spawned = false;//declare boolean flag

    while (alien_spawned == false) {//alien must spawn somewhere in the maze
        int randomX = (rand() % (SIZE_OF_MAZE-2) + 1);//random number between 1 and 19
        int randomY = (rand() % (SIZE_OF_MAZE-2) + 1);//random number between 1 and 19
        //int randomX = 1;//debug value
        //int randomY = 2;//debug value

        std::string comparison = array[randomX][randomY];//set string as the value in the array

        //std::cout << comparison << array[randomX][randomY] << std::endl;//debug statement

        //check is the random alien square in part of the maze path
        if (comparison == "⬜") {
            //update alien coordinates and place in the array
            *alien1X = randomX;
            *alien1Y = randomY;
            array[*alien1X][*alien1Y] = "👾";

            alien_spawned = true;//update boolean flag
        }
    }
}
...
int alien1_move(int *playerX, int *playerY, int *lives, int *alien1X, int *alien1Y, int* alien1Dir, std::string (&array)[SIZE_OF_MAZE][SIZE_OF_MAZE], bool* Has_Key) {
    ...
    bool alien1Moved = false;//declare boolean flag

    while (alien1Moved == false) {//alien must have moved to a valid square

        //allow the alien to travel in all valid square in one direction before changing to a new one
        if (*alien1Dir == -1) {
            *alien1Dir = rand() % 4;//random number from 0 to 3
        }

        switch (*alien1Dir) {
            case 0:{//if the number was 0
                changeX = *alien1X-1;//decrease the x value to make the alien go up in the maze
                //std::cout << "up\n";//debug statement
                break;
            }
            case 1:{//if the number was 1
                changeX = *alien1X+1;//increase the x value to make the alien go down in the maze
                //std::cout << "down\n";//debug statement
                break;
            }
            case 2:{//if the number was 2
                changeY = *alien1Y-1;//decrease the y value to make the alien go left in the maze
                //std::cout << "left\n";//debug statement
                break;
            }
            case 3:{//if the number was 3
                changeY = *alien1Y+1;//increase the y value to make the alien go right in the maze
                //std::cout << "right\n";//debug statement
                break;
            }
            default: {//as its a random number from 0 to 3 there cant be a number outside of bounds
                //std::cerr << "Invalid input" << std::endl;
                return 1;
            }
        }
        //ensure the tile the alien is moving to isn't a wall
        if (changeX > 0 && changeX < (SIZE_OF_MAZE-1) && changeY > 0 && changeY < (SIZE_OF_MAZE-1) && array[changeX][changeY] != "⬛") {
            if (*alien1X == 19 && *alien1Y == 19 && *Has_Key == false) {
                array[*alien1X][*alien1Y] = "🔒";//leave the correct symbol behind if the alien goes over the lock square
            } else if (*alien1X == 19 && *alien1Y == 19 && *Has_Key == true) {
                array[*alien1X][*alien1Y] = "🔐";//leave the correct symbol behind if the alien goes over the unlock square
            } else if (*alien1X == 1 && *alien1Y == 19 && *Has_Key == false) {
                array[*alien1X][*alien1Y] = "🔑";//leave the correct symbol behind if the alien goes over the key square
            } else if (*playerX == *alien1X && *playerY == *alien1Y) {
                array[*alien1X][*alien1Y] = "😀";//leave the correct symbol behind if the alien goes over the player square
            } else {
                array[*alien1X][*alien1Y] = "⬜️";//leave the path symbol behind
            }
            //update alien position in the array
            *alien1X = changeX;
            *alien1Y = changeY;
            array[*alien1X][*alien1Y] = "👾";
            alien1Moved = true;
    ...
    //check if the player was caught by the alien
    if (*playerX == *alien1X && *playerY == *alien1Y) {
        std::cout << "You have been caught by the alien" << std::endl;//tell the user
